...
if( dummyNum != NColloids ){
    simProg << "Number of colloids does not match number of colloids in file  *** \n Aborting" << std::endl; 
    abort();
}
...
for ( j = 0 ; j < npart ; ++ j ){   
    ...
    while ( flag == 1 && i < NColloids
        ...
        colloidRadSqr=pow(colloidRad[i],2.);
