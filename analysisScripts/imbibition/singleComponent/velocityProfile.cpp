...
////////////////////////////// DECLARATIONS /////////////////////////////
/////////////////////////////////////////////////////////////////////////
void writeMatToFile(double mat[][12], unsigned int, unsigned int, std::string &, unsigned int);  // write matrix to a file
...
int main(int argc, char** argv){
    ...
    double rhoSum[zBins][12];
    double vrSum[zBins][12];
    double vzSum[zBins][12];
    ...
    resetToZero(rhoSum, zBins, rBins);
    resetToZero(vrSum, zBins, rBins);
    resetToZero(vzSum, zBins, rBins);
    ...
    for (time=tStart;time<=tEnd;time+=tDiff){
        ...
        if( readPosVel.is_open() ){
            ...
            for (int j = 0 ; j<npart ; ++j){	
                ...
                if( (z_wrt_lowMen > 0.) && 
                    ...
                    rhoSum[zbin][rbin] += 1.;                 // add to the density
                    vrSum[zbin][rbin] +=  vr;                // add to the radial velocity
                    vzSum[zbin][rbin] +=  vz;                // add to the axial velocity
                }
        ...
        // write File
        writeMatToFile(rhoSum, zBins, rBins, rho_fname, count);
        writeMatToFile(vrSum, zBins, rBins, vr_fname, count);
        writeMatToFile(vzSum, zBins, rBins, vz_fname, count);
...
void writeMatToFile(double mat[][12], unsigned int m, unsigned int n, std::string &s, unsigned int count){

    std::ofstream fWrite;

    char fname[100];
    sprintf(fname, "./postProcessed/velocityAnalysis/%s_%d.dat", s.c_str(), count);
    fWrite.open(fname);
