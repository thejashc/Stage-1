...
if ( abs(mz) == 1.0 ) {
    ...
    double zOld                = particles[i].r.Z - particles[i].w.Z * dt;	// calculate the previous z-position
    double tApp                = ( particles[i].w.Z > 0. ) ? ( ( boxEdge[z] - zOld ) / particles[i].w.Z ) : ( -zOld / particles[i].w.Z ); 
    double tSep                = dt - tApp;				// time in which particle separates from top face of the box
